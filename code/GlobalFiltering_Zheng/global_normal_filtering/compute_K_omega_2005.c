                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    for (j = 0; j < nFaces; j++) 
    {
        ...
        for (i = 0; i < nNeighbors; i++)
        {
            ...
            distNormals = (normalsFace[x] - normalsFace[y]) * normalsFace[y]
                         + (normalsFace[pitch+x] - normalsFace[pitch+y]) * normalsFace[pitch+y]
                         + (normalsFace[2*pitch+x] - normalsFace[2*pitch+y]) * normalsFace[2*pitch+y];
            Wc = exp( - distCenters / (2 * sigmaC * sigmaC));
            Ws = exp( - distNormals * distNormals / (2 * sigmaS * sigmaS));
